...
class ExamServiceTest {
    ...
    private inner class FakeExamRepository : IExamRepository {
        ...
        override fun findById(id: UUID): ExamRecord? =
            exams.find { it.id == id }

        override fun findAll(): List<ExamRecord> =
            exams.toList()

        override fun findByTitle(title: String): List<ExamRecord> =
            exams.filter { it.title == title }

        override fun findByProfessor(professorId: UUID): List<ExamRecord> =
    ...
    @Test
    fun `createExam returns response with correct fields`() {
    ...
    @Test
    fun `createExam stores null description`() {
    ...
    @Test
    fun `createExam throws IllegalArgumentException when title is blank`() {
        assertThrows<IllegalArgumentException> {
            service.createExam(professorId, "  ", null, 90)
        }
    }

    @Test
    fun `createExam throws IllegalArgumentException when duration is zero`() {
        assertThrows<IllegalArgumentException> {
            service.createExam(professorId, "Networks", null, 0)
        }
    }

    @Test
    fun `createExam throws IllegalArgumentException when duration is negative`() {
        assertThrows<IllegalArgumentException> {
            service.createExam(professorId, "Networks", null, -1)
        }
    }
    ...
    @Test
    fun `getExamsByTitle returns all exams with that title`() {
    ...
    @Test
    fun `getExamsByTitle returns empty list when no exams match`() {
    ...
    @Test
    fun `getExamsByTitle throws IllegalArgumentException when title is blank`() {
        assertThrows<IllegalArgumentException> {
            service.getExamsByTitle("  ")
        }
    }
    ...
    @Test
    fun `getAllExams returns all exams`() {
    ...
    @Test
    fun `getAllExams returns empty list when no exams exist`() {
        val results = service.getAllExams()
    ...
    @Test
    fun `getExamById returns exam when it exists`() {
    ...
    @Test
    fun `getExamById throws NoSuchElementException when exam does not exist`() {
        assertThrows<NoSuchElementException> {
            service.getExamById(UUID.randomUUID())
        }
    }
}
